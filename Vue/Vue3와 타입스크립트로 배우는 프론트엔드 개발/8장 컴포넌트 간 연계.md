...
interface Emits {
  (event: "incrementPoint", id: number, e: MouseEvent): void;
}

const pointUp = (e: MouseEvent): void => {
  emit("incrementPoint", props.id, e);
};
...
```vue
<script setup lang="ts">
const handleIncrementPoint = (id: number, e: MouseEvent) => {
  console.log("id:", id);
  console.log("event:", e.type, e.clientX, e.clientY);
};
</script>
```
